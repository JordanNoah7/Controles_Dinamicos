...
SELECT  f.name AS foreign_key_name  
 ,OBJECT_NAME(f.parent_object_id, 5) AS table_name  
...
FROM Prueba.sys.foreign_keys AS f  
INNER JOIN Prueba.sys.foreign_key_columns AS fc   
ON f.object_id = fc.constraint_object_id   
WHERE f.parent_object_id = OBJECT_ID('Prueba.dbo.Table_1');

select f.name, f.parent_object_id, OBJECT_NAME(fc.parent_object_id, 5), OBJECT_NAME(fc.parent_column_id) from Prueba.sys.foreign_key_columns fc join Prueba.sys.foreign_keys f on fc.parent_object_id = f.parent_object_id
